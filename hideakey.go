...
func (s *Store) dblocker(passphrase, key, padlock []byte) []byte {
	...
	if passphrase == nil { passphrase = hashBytes(randBytes(256)) }
	if key == nil { key = hashBytes(randBytes(256)) }
	...
	if inject > slotSize-lockLen { inject = slotSize - lockLen}
	...
	if inject+keyLen > slotSize { inject = slotSize - keyLen}
	...
	for i, v := range lock { s.set("KVLite", "X"+strconv.Itoa(i), v, _reserved) }
	...
	if padlock != nil { s.set("KVLite", "X"+strconv.Itoa(xSlots+3), randBytes(slotSize), _reserved) }
...
func (s *Store) dbunlocker(padlock []byte) (err error) {
	...
	if count < xSlots+4 { padlock = nil }
	
	XMsg := make([][]byte, count)
	
	for i := 0; i < count; i++ { s.Get("KVLite", "X"+strconv.Itoa(i), &XMsg[i]) }
	...
	tryKey := func(rkey []byte) ([]byte) {
		...
		if bytes.Compare(key1[0:32], decrypted[0:32]) == 0 { return key1 }
		return nil
	}
	...
	tryPass := func(testPass []byte, origin int) (passphrase, key []byte) {
		...
		for i := 0; i < xSlots; i++ {
			...
			for n := 0; n <= slotSize-44; n++ {
				...
				if len(passphrase) != 33 { continue }
				deCrypted := decrypt(vPass, passphrase[0:32])
				if key := tryKey(deCrypted[0:32]); key != nil { return passphrase[0:32], key[0:32] }
				continue
			}
		}
		return nil, nil
	...
	for a := 0; a < xSlots; a++ {
		for b := 0; b < slotSize; b = b + keyLen {
			...
			if slotEnd < keyLen { b = slotSize - keyLen }
